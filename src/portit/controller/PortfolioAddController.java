...
public class PortfolioAddController implements Controller {
	...
	public String[] execute(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		for (String key : formData.keySet()) {
			if (key.startsWith("pf_tags_") && "language".equals(key.substring(8))) {
				String[] pf_tags_language = formData.get(key).split(", ");
				...
				String[] pf_tags_tool = formData.get(key).split(", ");
				...
				String[] pf_tags_field = formData.get(key).split(", ");
				...
				String[] pf_coworker = formData.get(key).split(", ");
		...
		try {
			portfolio.setPf_id(generateId())
					.setPf_title(formData.get("pf_title"))
