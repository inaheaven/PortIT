...
<jsp:useBean id="searchDao" class="portit.model.dao.SearchDao" />
	...
	<section class="container">
		<section class="wrapper site-min-height">
			<div class="col-md-12 mt search" id="searchPf">
				...
				<!-- END - 조건 검색 box -->
<%	
 	List list = (List)request.getAttribute("port_list");
 ...
 %>
		...
		<div align="center">
			<% if(nowBlock > 0){%>
			<a
				href="/page?page=pfSearch?nowBlock=<%=nowBlock-1%>&nowPage=<%=pagePerBlock*(nowBlock+1)%>">이전<%=pagePerBlock%>개
			</a>
			<% }%>
			:::
			<%
 		...
 		%>
			<a
				href="/page?page=pfSearch?nowPage=<%=(nowBlock*pagePerBlock)+i%>&nowBlock=<%=nowBlock%>"><%= (nowBlock*pagePerBlock)+i+1%></a>&nbsp;&nbsp;&nbsp;
			<%
 			}
 		%>
			:::
			...
			<a
				href="/page?page=pfSearch?nowBlock=<%=nowBlock+1%>&nowPage=<%=pagePerBlock*(nowBlock+1)%>">다음<%=pagePerBlock%>개
			</a>
			<% }%>
		</div>
